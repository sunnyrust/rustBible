...
原本是想对之前的`TagModel`增加一个`is_parent`的字段，为了直接判断是不是`pid`方便。
...
原来的想法比较简单，就是在插入每行数据到`TagNode`的时候，直接搜索数据库，判断一下这个`id`是不是一个`pid`。
...
明显和这个报错没有任何关系，我做了各种猜测个和假设，都是不正确的，最后我发现了这么一个情况
...
把函数调用写在这句上面就ok，写在下面就over。这个时候张汉东大佬的一句话说出了事实的真相：

![](./7.12.3.jpg)

虽然，我现在还没有完全理解这句话的真正含义，但是我修改了代码，实现了目标。
...
pub async fn get_tag_tree<'a>(state: &'a DbState) -> Result<Vec<TagModel>> {
    ...
    if !b_have_key{
        ...
        let rows = sqlx::query_as::<_, Model>(&sql)
            ...
            .await
            .unwrap();
        sql=format!("SELECT distinct(pid) as id FROM {} order by pid ASC;",get_table_name());
