...
<body>
  ...
  <div style="display: flex;">
    <div style="flex: 0 0 30%; ">
      <!-- 左边内容 -->
      <form id="edit_form" name="form" method="post" action="./">
        <input type="text" id="id" name="id" value="{{id}}" readonly="readonly" />
        <input type="text" id="pid" name="pid" value="{{pid}}" readonly="readonly" />
        <br />
...
fn set_html(stack:&mut Stack<TagModel>,tml:&TagModel,html:String,have_link:bool)->String{
    ...
    if judge_push(&stack,&tml){
        ...
        if have_link{
            s=format!("<ul><li id='tree_{}'><a href='./find/{}'>{}</a>",tml.id,tml.id,tml.name);
        }else{
            s=format!("<ul><li id='tree_{}'>{}",tml.id,tml.name);
        }
        ...
        if !stack.is_empty(){
            ...
            let mut s=String::new();
            if have_link{
                s=format!("<li id='tree_{}'><a href='./find/{}'>{}</a>",tml.id,tml.id,tml.name);
            }else{
                s=format!("<li id='tree_{}'>{}",tml.id,tml.name);
            }
